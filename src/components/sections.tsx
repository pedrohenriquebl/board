...
function SectionRoot({ className, ...props }: SectionRootProps) {
    return (
        <div className={twMerge("bg-navy-800 rounded-xl border-[0.5px] border-navy-500 pt-3 flex flex-col gap-1 relative mb-10", className)} {...props} />
    )
}
...
function SectionContent({ className, ...props }: SectionContentProps) {
    return (
        <div
            className={twMerge(
                "flex flex-col gap-2.5 overflow-y-auto p-3 absolute inset-0 top-10 mb-2.5 mr-1",
                "scrollbar-thin scrollbar-track-navy-800 scrollbar-thumb-navy-600",
                className
            )}
            {...props}
        />
